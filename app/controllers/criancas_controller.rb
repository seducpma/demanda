         ...
         @criancas_pre = Crianca.find( :all,:conditions => ["nome like ?  and  (grupo_id = 6 or grupo_id = 7  or (grupo_id = 5 and  regiao_id= 999 )) and recadastrada > 0" , "%" + params[:search1].to_s + "%"],:order => 'nome ASC, unidade_id ASC')
          render :update do |page|
             ...
             #@criancas_pre = Crianca.find( :all,:conditions => ["nascimento <= ?  and recadastrada > 0 and (grupo_id = 6)and (status='NA_DEMANDA')" ,data_pre ],:order => 'grupo_id ASC, nome ASC')
                 @criancas_pre  = Crianca.find( :all,:conditions => ["recadastrada > 0 and ( grupo_id =6 )and (status='NA_DEMANDA') and created_at > '2021-09-09 00:00:00'"  ],:order => 'nome ASC, grupo_id ASC')
                 @criancas_pre2 = Crianca.find( :all,:conditions => ["recadastrada > 0 and ( grupo_id =6 )and (status='NA_DEMANDA') and created_at < '2021-09-09 00:00:00'" ],:order => 'nome ASC, grupo_id ASC')
                 ...
                 t=0
                  render :update do |page|
                     ...
                     @criancas_pre  = Crianca.find( :all,:conditions => ["recadastrada > 0 and ( grupo_id =7 )and (status='NA_DEMANDA') and created_at > '2021-09-09 00:00:00'"  ],:order => 'nome ASC, grupo_id ASC')
                     @criancas_pre2 = Crianca.find( :all,:conditions => ["recadastrada > 0 and ( grupo_id =7 )and (status='NA_DEMANDA') and created_at < '2021-09-09 00:00:00'" ],:order => 'nome ASC, grupo_id ASC')
